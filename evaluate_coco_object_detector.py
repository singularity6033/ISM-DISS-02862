...
TP_FN = [[] for i in range(NUM_CLASS)]
# Precision: TP / (TP + FP)
...
for (i, TestImagePath) in enumerate(TestImagePaths):
    ...
    for annInfo in annInfos:
        ...
        label_id_gt = annInfo['category_id']
        ...
        gtBoxes.append((xMin, yMin, xMax, yMax, label_id_gt))
    ...
    for (x, y, w, h) in rects[random.sample(rd_index, 200)]:
...
plt.ylabel("AP")
plotPath = os.path.sep.join([config.PLOTS_PATH, "coco_ap_3.png"])
